$(function () {
    ...
    $('#rotateLButton').click(function () {
        var val = getInputHexValue();

        var out = [];
        for (var i = 7; i >= 0; i--) //new bytes, pocessing each new line
        {
            var newbyte = 0x00;
            for (var j = 0; j < 8; j++) // old bytes, processing columns to transform in line
            {
                var oldbyte = val.substr(j * 2, 2);
                oldbyte = parseInt(oldbyte, 16);
                oldbyte >>= i; // rotate the number of the old column (new line)
                oldbyte &= 0x01;
                oldbyte <<= j;
                newbyte |= oldbyte;
            ...
            newbyte = newbyte.toString(16);
            newbyte = ('0' + newbyte).substr(-2);
            out.push(newbyte);
        }
        val = out.join('');
        $hexInput.val(val);
    ...
    $('#rotateRButton').click(function () {
        ...
        for (var i = 7; i >= 0; i--) //new bytes, pocessing each new line
        {
            ...
            for (var j = 0; j < 8; j++) // old bytes, processing columns to transform in line
            {
                var oldbyte = val.substr((7-j) * 2, 2); // o index é da esquerda para a direita
                oldbyte = parseInt(oldbyte, 16);

                oldbyte >>= (7-i); // rotate the number of the old column (new line)
                oldbyte &= 0x01;
                oldbyte <<= j;
                newbyte |= oldbyte;
            ...
            newbyte = newbyte.toString(16);
            newbyte = ('0' + newbyte).substr(-2);
            out.push(newbyte); // coloca na ordem : 01 02 03 04 05 ...
        }
        val = out.join('');
        $hexInput.val(val);
        hexInputToLeds();
    ...
    $('#flipVButton').click(function () {
        ...
        {
            var newbyte;
            newbyte = val.substr(i * 2, 2); // o index é da esquerda para a direita
            newbyte = ('0' + newbyte).substr(-2);
            out.push(newbyte); // coloca na ordem : 01 02 03 04 05 ...
        }
        val = out.join('');
    ...
    $('#flipHButton').click(function () {
        ...
        {
            var newbyte = 0x00;
            var oldbyte = val.substr((7-i) * 2, 2); // o index é da esquerda para a direita
            oldbyte = parseInt(oldbyte, 16);

            for (var j = 0; j < 8; j++) // old bytes, processing columns to transform in line
            {   
                newbyte <<= 1;
                newbyte |= (0x01 & oldbyte);
                oldbyte >>= 1;
                
            }
            newbyte = newbyte.toString(16);
            newbyte = ('0' + newbyte).substr(-2);
            out.push(newbyte); // coloca na ordem : 01 02 03 04 05 ...  (i=7) (i=6) (i=5) ...
        }
        val = out.join('');
        $hexInput.val(val);
        hexInputToLeds();
